...
const mapMedia = (m: any): MomentMedia => ({
  id: m.id,
...
export async function getMoments(category?: string) {
  ...
  const { data: media } = ids.length
    ? await supabase.from("moment_media").select("*").in("moment_id", ids).order("sort_order")
    : { data: [] };
  ...
  return (data ?? []).map((m: any) => ({
    ...
    mediaUrl: mediaMap.get(m.id)?.media_url ?? null,
  }));
}
...
export async function getMomentDetail(id: string): Promise<MomentDetail | null> {
  ...
  const [{ data: media }, { count: likeCount }, { count: commentCount }, { data: comments }, { data: journey }, { data: currentUser }] = await Promise.all([
    ...
    supabase.from("journeys").select("status").eq("moment_id", id).maybeSingle(),
    supabase.auth.getUser(),
  ]);

  const userId = currentUser.user?.id;
  const { data: like } = userId ? await supabase.from("likes").select("user_id").eq("moment_id", id).eq("user_id", userId).maybeSingle() : { data: null };
  const { data: follow } = userId && userId !== m.user_id
    ? await supabase.from("follows").select("follower_id").eq("follower_id", userId).eq("following_id", m.user_id).maybeSingle()
    : { data: null };
  ...
  return {
    ...
    journey: { status: journey?.status ?? null },
    comments: (comments ?? []).map((c: any): Comment => ({
      id: c.id,
      userId: c.user_id,
      ...
      author: c.profiles ? mapProfile(c.profiles) : undefined,
    })),
  };
}
