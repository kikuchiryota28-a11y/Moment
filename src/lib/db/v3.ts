...
export async function getTodayMoment():Promise<DailyMoment>{
 ...
 const[{count},my]=await Promise.all([sb.from("results").select("id",{count:"exact",head:true}).eq("daily_moment_id",d.id),user?sb.from("results").select("id").eq("daily_moment_id",d.id).eq("user_id",user.id).maybeSingle():Promise.resolve({data:null})]);
...
export async function getResults(dailyMomentId:string,userId?:string):Promise<Result[]>{const sb=await createClient();const{data,error}=await sb.from("results").select("id,daily_moment_id,user_id,result_type,text_content,choice_value,why,country_code,city,created_at,profiles!results_user_id_fkey(username,display_name,avatar_url),result_media(media_url)").eq("daily_moment_id",dailyMomentId).order("created_at",{ascending:true}).limit(100);if(error)throw error;const rows=(data??[]) as unknown as ResultRow[];return selectReveal(rows.filter(r=>r.user_id!==userId)).map(mapResult);}
...
function selectReveal(rows:ResultRow[]){const picked:ResultRow[]=[];const take=(p:(r:ResultRow)=>boolean)=>{const i=rows.findIndex(r=>!picked.includes(r)&&p(r));if(i>=0)picked.push(rows[i]);};take(r=>r.result_type==="choice");take(r=>r.result_type!==picked[0]?.result_type);take(r=>Boolean(r.country_code)&&r.country_code!==picked[0]?.country_code);take(r=>r.result_type==="photo"||r.result_type==="video");take(r=>r.result_type==="text"||r.result_type==="combination");for(const r of rows){if(picked.length>=5)break;if(!picked.includes(r))picked.push(r)}return picked.slice(0,5);}
