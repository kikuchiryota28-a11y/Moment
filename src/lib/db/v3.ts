...
export async function getTodayMoment(): Promise<DailyMoment> {
  ...
  if (!row) {
    const { data: directRow } = await sb
      .from("daily_moments")
      .select("id,moment_date,prompt,status,first_mover_id,started_at,ends_at")
      .eq("moment_date", new Date().toISOString().slice(0, 10))
      .maybeSingle();
  ...
  const status =
    d.status === "LIVE" && ends <= now
      ? "ENDED"
      : d.status === "LIVE" && ends - now < 3 * 60 * 60 * 1000
      ? "ENDING"
      : d.status;
  ...
  const [{ count }, my] = await Promise.all([
    sb
      ...
      .eq("daily_moment_id", d.id)
      .eq("moderation_status", "VISIBLE"),
    user
      ? sb.from("results").select("id").eq("daily_moment_id", d.id).eq("user_id", user.id).maybeSingle()
      : Promise.resolve({ data: null }),
  ]);

  return {
    id: d.id,
    momentDate: d.moment_date,
    prompt: d.prompt,
    status: status as DailyMoment["status"],
    firstMoverId: d.first_mover_id,
    startedAt: d.started_at,
    endsAt: d.ends_at,
    participantCount: count ?? 0,
    myResultId: my.data?.id ?? null,
  };
}
...
export async function getResults(dailyMomentId: string, userId?: string): Promise<Result[]> {
  ...
  const { data, error } = await sb
    .from("results")
    .select(
      "id,daily_moment_id,user_id,result_type,text_content,choice_value,why,country_code,city,created_at,profiles!results_user_id_fkey(username,display_name,avatar_url),result_media(media_url)"
    )
    .eq("daily_moment_id", dailyMomentId)
    .eq("moderation_status", "VISIBLE")
  ...
  return selectReveal(rows.filter((r) => r.user_id !== userId)).map(mapResult);
}

export async function getWorldArchive(limit = 24) {
  const sb = await createClient();
  const { data, error } = await sb
    .from("daily_moments")
    .select("id,moment_date,prompt,status")
    .lt("moment_date", new Date().toISOString().slice(0, 10))
...
export async function getResultById(id: string): Promise<Result | null> {
  ...
  if (error) throw error;
  return data ? mapResult(data as unknown as ResultRow) : null;
}
...
function selectReveal(rows: ResultRow[]) {
  ...
  const take = (p: (r: ResultRow) => boolean) => {
    const i = rows.findIndex((r) => !picked.includes(r) && p(r));
    if (i >= 0) {
      const row = rows[i];
      picked.push(row);
      usedTypes.add(row.result_type);
...
function mapResult(r: ResultRow): Result {
  return {
    id: r.id,
    dailyMomentId: r.daily_moment_id,
    userId: r.user_id,
    ...
    city: r.city,
    mediaUrl: r.result_media?.[0]?.media_url ?? null,
