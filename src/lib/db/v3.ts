...
function selectReveal(rows:ResultRow[]){const picked:ResultRow[]=[];const usedTypes=new Set<string>();const usedCountries=new Set<string>();const take=(p:(r:ResultRow)=>boolean)=>{const i=rows.findIndex(r=>!picked.includes(r)&&p(r));if(i>=0){const row=rows[i];picked.push(row);usedTypes.add(row.result_type);const country=row.country_code;if(country)usedCountries.add(country);}};
take(r=>r.result_type==="choice");take(r=>!usedTypes.has(r.result_type));take(r=>Boolean(r.country_code)&&!usedCountries.has(r.country_code));take(r=>r.result_type==="photo"||r.result_type==="video");take(r=>r.result_type==="text"||r.result_type==="combination");
