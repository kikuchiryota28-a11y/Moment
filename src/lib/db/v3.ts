...
function selectReveal(rows:ResultRow[]){const picked:ResultRow[]=[];const usedTypes=new Set<string>();const usedCountries=new Set<string>();const take=(p:(r:ResultRow)=>boolean)=>{const i=rows.findIndex(r=>!picked.includes(r)&&p(r));if(i>=0){picked.push(rows[i]);usedTypes.add(rows[i].result_type);if(rows[i].country_code)usedCountries.add(rows[i].country_code)}};
