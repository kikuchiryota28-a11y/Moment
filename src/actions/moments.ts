...
function validateOwnedMediaUrls(media: CreateMomentInput["media"], userId: string): boolean {
  return media.every((item) => {
    const path = storagePathFromPublicUrl(item.url, "moment-media");
...
export async function createMoment(input: CreateMomentInput): Promise<ActionResult<{ id: string }>> {
  ...
  try {
    ...
    const { data: moment, error: momentError } = await supabase.from("moments").insert({
      user_id: user.id, title: input.title.trim(), description: input.description.trim(), why: input.why?.trim() || null,
      category: input.category, location_name: input.location?.trim() || null, duration_minutes: input.durationMinutes || null,
      estimated_cost: input.estimatedCost ?? null, experience_note: input.experienceNote?.trim() || null,
      rating: input.rating, would_do_again: input.wouldDoAgain,
    }).select("id").single();
...
export async function updateMoment(input: UpdateMomentInput): Promise<ActionResult<{ id: string }>> {
  ...
  try {
    ...
    const { error: updateError } = await supabase.from("moments").update({
      title: input.title.trim(), description: input.description.trim(), why: input.why?.trim() || null, category: input.category,
      location_name: input.location?.trim() || null, duration_minutes: input.durationMinutes ?? null, estimated_cost: input.estimatedCost ?? null,
      experience_note: input.experienceNote?.trim() || null, rating: input.rating, would_do_again: input.wouldDoAgain,
    }).eq("id", input.momentId).eq("user_id", user.id);
    ...
    const { error: deleteMediaError } = await supabase.from("moment_media").delete().eq("moment_id", input.momentId).not("media_url", "in", `(${input.media.map((item) => `"${item.url.replaceAll('"', '""')}"`).join(",")})`);
    ...
    if (removedPaths.length) {
      const { error: storageError } = await supabase.storage.from("moment-media").remove(removedPaths);
      if (storageError) console.error("updateMoment old media cleanup failed", storageError);
    }
    revalidatePath("/");
...
export async function deleteMoment(momentId: string): Promise<ActionResult<undefined>> {
  ...
  try {
    ...
    const [{ data: moment, error: momentError }, { data: media, error: mediaError }] = await Promise.all([
      supabase.from("moments").select("id,user_id").eq("id", momentId).maybeSingle(),
      supabase.from("moment_media").select("media_url").eq("moment_id", momentId),
    ]);
    ...
    if (paths.length) {
      const { error: storageError } = await supabase.storage.from("moment-media").remove(paths);
      if (storageError) console.error("deleteMoment storage cleanup failed", storageError);
    }
