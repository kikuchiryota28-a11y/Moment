...
export async function tryMoment(momentId: string): Promise<ActionResult<{ status: JourneyStatus }>> {
  ...
  try {
    ...
    if (momentError) { console.error("tryMoment moment lookup failed", momentError); return { success: false, error: "Momentを取得できませんでした。", code: "MOMENT_LOOKUP_FAILED" }; }
    ...
    const [{ data: existing, error: existingError }, { data: cover }] = await Promise.all([
      supabase.from("journeys").select("id,status").eq("user_id", user.id).eq("moment_id", momentId).maybeSingle(),
      supabase.from("moment_media").select("media_url").eq("moment_id", momentId).order("sort_order").limit(1).maybeSingle(),
    ]);
    if (existingError) { console.error("tryMoment journey lookup failed", existingError); return { success: false, error: "Journeyを確認できませんでした。", code: "JOURNEY_LOOKUP_FAILED" }; }
    if (existing) {
    ...
    if (error || !data) { console.error("tryMoment journey insert failed", error); return { success: false, error: "Journeyへの追加に失敗しました。", code: "JOURNEY_INSERT_FAILED" }; }
...
export async function startJourney(journeyId: string): Promise<ActionResult<{ status: "TRYING" }>> {
  return transitionJourney(journeyId, "PLANNED", "TRYING", { status: "TRYING", started_at: new Date().toISOString() });
}

export async function completeJourney(journeyId: string): Promise<ActionResult<{ status: "COMPLETED" }>> {
...
async function transitionJourney<T extends TransitionStatus>(journeyId: string, expected: "PLANNED" | "TRYING", nextStatus: T, patch: Record<string, string>): Promise<ActionResult<{ status: T }>> {
  if (!journeyId || !/^[0-9a-f-]{36}$/i.test(journeyId)) return { success: false, error: "Invalid Journey.", code: "INVALID_JOURNEY_ID" };
  try {
    const supabase = await createClient();
    const { data: { user }, error: authError } = await supabase.auth.getUser();
    if (authError || !user) return { success: false, error: "Authentication required.", code: "AUTH_REQUIRED" };
    ...
    if (journeyError) { console.error("transitionJourney lookup failed", journeyError); return { success: false, error: "Journeyを取得できませんでした。", code: "JOURNEY_LOOKUP_FAILED" }; }
    ...
    if (error) { console.error("transitionJourney update failed", error); return { success: false, error: "Journeyの更新に失敗しました。", code: "JOURNEY_UPDATE_FAILED" }; }

    revalidatePath("/journey");
    if (journey.moment_id) revalidatePath(`/moment/${journey.moment_id}`);
