...
export async function planJourney(journeyId: string, plannedAt: string): Promise<ActionResult<{ plannedAt: string }>> {
  ...
  try {
    ...
    revalidatePath("/journey");
    return { success: true, data: { plannedAt: data.planned_at } };
  } catch (error) {
    console.error("planJourney unexpected failure", error);
    return { success: false, error: "予定日の保存に失敗しました。", code: "PLAN_FAILED" };
  }
}
...
export async function recordExperience(formData: FormData): Promise<ActionResult<{ status: "COMPLETED" }>> {
  ...
  try {
    ...
    if (updateError) {
      await supabase.storage.from(EXPERIENCE_BUCKET).remove([uploadedPath]);
      return { success: false, error: "Experienceの保存に失敗しました。", code: "EXPERIENCE_SAVE_FAILED" };
    }
    ...
    console.error("recordExperience unexpected failure", error);
    if (uploadedPath) {
      try {
        const supabase = await createClient();
        await supabase.storage.from(EXPERIENCE_BUCKET).remove([uploadedPath]);
      } catch (cleanupError) {
        console.error("recordExperience cleanup failed", cleanupError);
      }
    }
    return { success: false, error: "Experienceの保存に失敗しました。", code: "EXPERIENCE_SAVE_FAILED" };
  }
