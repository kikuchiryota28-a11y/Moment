  ...
  return (
    <main className="moment-container py-8 pb-28 md:py-10">
      <header className="pt-10 pb-8 md:pt-16">
        ...
        <h1 className="moment-display mt-3 max-w-2xl text-5xl md:text-7xl">Where have you been?</h1>
      </header>
      <section className="border-y border-[var(--color-line)]">
        {merged.length || journeyOnly.length ? (
          <div className="divide-y divide-[var(--color-line)]">
            {merged.map((r: any) => (
              <Link key={r.id} href={r.phase === "trying" ? `/moment/${r.daily_moment_id}` : `/moment/${r.daily_moment_id}/reveal`} className="group flex min-h-24 items-center justify-between gap-5 rounded-[22px] border border-[var(--color-line)]/65 bg-white/30 px-5 py-5 backdrop-blur-xl md:px-6">
