  ...
  return (
    <main className="moment-container py-8 pb-28 md:py-12 md:pb-16">
      ...
      <section className="py-10">
        <div className="mb-5"><p className="moment-eyebrow text-[var(--color-muted-ink-value)]">Your record</p><h2 className="moment-display mt-2 max-w-2xl text-4xl md:text-6xl">Moments you actually lived.</h2></div>
        {results.length ? (
          <div className="divide-y divide-[var(--color-line-value)] border-y border-[var(--color-line-value)]">
            {results.map((r: any) => (
              <Link key={r.id} href={`/moment/${r.daily_moment_id}/reveal`} className="group flex min-h-28 items-center justify-between gap-5 py-5">
          ...
          <Card variant="outlined" className="p-10 text-center"><h3 className="moment-display text-4xl">Nothing here yet.</h3><p className="mx-auto mt-3 max-w-md text-sm text-[var(--color-muted-ink-value)]">Your first real action will become the first entry in this record.</p><Link href="/" className="mt-6 inline-flex"><Button size="lg">Open today's Moment</Button></Link></Card>
        )}
      </section>
    </main>
  );
