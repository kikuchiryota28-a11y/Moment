...
export function SettingsPanel({ initial, email }: { initial: SettingsState; email: string }) {
  ...
  return <div className="py-5 sm:py-10">
    ...
    <Section title="ACCOUNT">
      ...
      <Row label="Log out" description="End this session"><form action={signOut}><button disabled={isPending} className="min-h-10 rounded-xl border border-[#ded8ce] px-4 text-sm font-bold disabled:opacity-50">Log out</button></form></Row>
      <Row label="Delete account" description="Permanently remove your MOMENT account"><button onClick={() => setShowDelete(true)} disabled={isPending} className="min-h-10 rounded-xl border border-red-200 px-4 text-sm font-bold text-red-700 disabled:opacity-50">Delete</button></Row>
    </Section>
    <Section title="PREFERENCES">
      <Row label="Appearance" description="System, light, or dark"><div className="grid grid-cols-3 gap-2">{(["system","light","dark"] as const).map((value) => <button key={value} disabled={isPending} onClick={() => chooseTheme(value)} aria-pressed={theme === value} className={`min-h-10 rounded-xl border px-3 text-xs font-bold capitalize disabled:opacity-50 ${theme === value ? "border-[#171614] bg-[#171614] text-white" : "border-[#ded8ce] bg-white/60"}`}>{value}</button>)}</div></Row>
    ...
    <Section title="PRIVACY & DATA">
      <VisibilityRow label="Activity visibility" value={state.activityVisibility} disabled={isPending} onChange={(value) => setVisibility("activityVisibility", value)} description="Controls whether your published Moments appear publicly." />
      <VisibilityRow label="Experience visibility" value={state.experienceVisibility} disabled={isPending} onChange={(value) => setVisibility("experienceVisibility", value)} description="Controls whether completed Journey activity is publicly readable." />
    </Section>
...
function VisibilityRow({ label, description, value, disabled, onChange }: { label: string; description: string; value: Visibility; disabled?: boolean; onChange: (value: Visibility) => void }) { return <Row label={label} description={description}><div className="grid grid-cols-2 gap-2">{(["public","private"] as const).map((item) => <button key={item} disabled={disabled} onClick={() => onChange(item)} aria-pressed={value === item} className={`min-h-10 rounded-xl border px-3 text-xs font-bold capitalize disabled:opacity-50 ${value === item ? "border-[#171614] bg-[#171614] text-white" : "border-[#ded8ce] bg-white/60"}`}>{item}</button>)}</div></Row>; }
