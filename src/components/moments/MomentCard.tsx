...
export function MomentCard({ moment, author, mediaUrl }: { moment: Moment; author: Profile; mediaUrl: string | null }) {
  return <article className="overflow-hidden rounded-[28px] border border-[#ded8ce] bg-white/80 shadow-[0_12px_40px_rgba(30,25,15,.06)]">
    ...
    <div className="space-y-4 p-5 sm:p-6">
      ...
      <div className="flex flex-wrap gap-3 text-xs font-semibold text-[#777269]">{moment.locationName && <span className="flex items-center gap-1"><MapPin size={14}/>{moment.locationName}</span>}{moment.durationMinutes && <span className="flex items-center gap-1"><Clock3 size={14}/>{moment.durationMinutes} min</span>}<span className="flex items-center gap-1"><Star size={14} fill="currentColor"/>{moment.rating}/5</span></div>
      <div className="flex items-center justify-between gap-4 border-t border-[#eee8df] pt-4"><span className="text-xs font-bold text-[#777269]">{moment.wouldDoAgain ? "Would do it again" : "One-time experience"}</span><div className="w-32"><TryButton momentId={moment.id}/></div></div>
    </div>
  </article>;
}
