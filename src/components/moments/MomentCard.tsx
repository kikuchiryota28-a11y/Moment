...
import type { JourneyStatus, Moment, Profile } from "@/types/moment";
import { TryButton } from "./TryButton";
import { Avatar } from "@/components/shared/Avatar";

export function MomentCard({ moment, author, mediaUrl, journeyStatus }: { moment: Moment; author: Profile; mediaUrl: string | null; journeyStatus?: JourneyStatus | null }) {
  const statusLabel = journeyStatus === "COMPLETED" ? "Experienced" : journeyStatus ? "In your Journey" : "Try this";
  return <article className="overflow-hidden rounded-[28px] border border-[#ded8ce] bg-white/80 shadow-[0_12px_40px_rgba(30,25,15,.06)]">
    ...
    <div className="space-y-4 p-5 sm:p-6">
      ...
      <div className="flex flex-wrap gap-3 text-xs font-semibold text-[#777269]">{moment.locationName && <span className="flex items-center gap-1"><MapPin size={14}/>{moment.locationName}</span>}{moment.durationMinutes && <span className="flex items-center gap-1"><Clock3 size={14}/>{moment.durationMinutes} min</span>}<span>{statusLabel}</span></div>
      <div className="flex items-center justify-between gap-4 border-t border-[#eee8df] pt-4"><span className="max-w-[55%] text-xs font-bold text-[#777269]">{moment.why ?? "A small idea worth turning into a real experience."}</span><div className="w-32"><TryButton momentId={moment.id} initialStatus={journeyStatus}/></div></div>
    </div>
  </article>;
}
