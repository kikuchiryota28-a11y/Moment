...
export function MomentPhysicsScene({ 
  ...
  return (
    <section className="fixed inset-0 z-0 h-[100dvh] w-full overflow-hidden bg-[var(--color-canvas)]" aria-label={`MOMENT interactive space — ${phase}`}>
      <Canvas 
        dpr={[1, 1.7]} 
      ...
      <div className="pointer-events-none absolute left-5 top-5 z-10 text-[10px] font-black uppercase tracking-[0.24em] text-[var(--color-muted-ink)] sm:left-8 sm:top-8">
        MOMENT / 03
      </div>
    </section>
  );
