...
export function JourneyCard({ journey }: { journey: Journey }) {
  ...
  function run(action: () => Promise<{ success: boolean; error?: string }>, onSuccess: () => void) {
    ...
    startTransition(async () => {
      ...
      if (result.success) onSuccess(); else setError(result.error ?? "更新に失敗しました。");
    });
  }
  ...
  return <article className="rounded-3xl border border-[#ded8ce] bg-white/70 p-4 sm:p-5">
    <div className="flex gap-4">
      ...
      <div className="min-w-0 flex-1">
        ...
        {status === "PLANNED" && <div className="mt-4 rounded-2xl border border-[#eee8df] bg-white/60 p-3"><p className="text-xs font-bold">When will you do it?</p><div className="mt-2 flex flex-col gap-2 sm:flex-row"><input type="datetime-local" value={plannedAt} onChange={(e) => setPlannedAt(e.target.value)} className="min-h-10 flex-1 rounded-xl border border-[#ded8ce] bg-white px-3 text-xs outline-none focus:border-[#ef6b35]"/><button onClick={savePlan} disabled={isPending} className="rounded-xl bg-[#171614] px-4 py-2.5 text-xs font-black text-white disabled:opacity-50">{isPending ? "Saving…" : "Plan it"}</button></div></div>}
