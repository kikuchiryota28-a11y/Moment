...
export function JourneyCard({ journey }: { journey: Journey }) {
  ...
  return <article className="flex gap-4 rounded-3xl border border-[#ded8ce] bg-white/70 p-4">
    ...
    <div className="min-w-0 flex-1">
      <Link href={`/moment/${journey.momentId}`} className="font-black hover:underline">{journey.moment?.title ?? "Untitled Moment"}</Link>
      ...
      {status === "COMPLETED" && <Link href="/create" className="mt-4 inline-block text-xs font-black underline">Share your Moment</Link>}
    </div>
  </article>;
}
