...
type Action = (formData: FormData) => Promise<{ ok: false; error: string } | never>;

export function AuthForm({ action, mode }: { action: Action; mode: "login" | "signup" }) {
  const [state, formAction, pending] = useActionState(async (_prev: { error?: string }, formData: FormData) => action(formData), {});
