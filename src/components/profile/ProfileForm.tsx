...
export function ProfileForm({ profile }: Props) {
  ...
  return <div className="py-5 sm:py-10">
    ...
    <section className="mt-8"><p className="text-xs font-black uppercase tracking-[0.16em] text-[#777269]">Profile Photo</p><div className="mt-4 flex items-center gap-4">
      <div className="relative h-20 w-20 overflow-hidden rounded-full bg-[#171614]">{avatarUrl ? <Image src={avatarUrl} alt="Profile photo" fill sizes="80px" className="object-cover" unoptimized={avatarUrl.startsWith("blob:")} /> : <div className="flex h-full w-full items-center justify-center text-2xl font-black text-white">{form.displayName.slice(0,1).toUpperCase()}</div>}</div>
